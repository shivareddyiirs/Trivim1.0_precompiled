...
drr = Dir.chdir('.\..')
drr = Dir.getwd
temp_display_file = File.open(drr+"/3d-modelling/temp_for_display.txt","w")	
tempfile = drr+"/curr_proj.txt"

base_address = IO.readlines(tempfile)
#Dir.chdir("C:/pSApp/input")
Dir.chdir(base_address[0]+"/input")
count = 0
x = Dir.pwd
y = Dir.entries(x)
for i in 2..y.length
...
for i in 1..count
	...
	for j in 1..floors
		...
		output_address = IO.readlines(drr+"/curr_proj.txt")[0]+"/output/build"
